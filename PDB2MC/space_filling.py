...
def run_mode(config_data, pdb_name, rounded, mc_dir, atom_df, hetatom_df, hetatm_bonds):
    ...
    if not x:
       ...
       # mcf.create_minecraft_functions(output_df, pdb_branch, False, mc_dir, config_data['atoms'], replace=True)
        ...
        mcf.create_nbt(point_df, pdb_branch, air=False, dir=mc_dir, blocks=config_data['atoms'])
        mcf.create_nbt(output_df, pdb_surface, air=False, dir=mc_dir, blocks=config_data['atoms'])
        ...
        if config_data["show_hetatm"]:
            if hetatom_df is not None:
                ...
                mcf.create_nbt(aligned_hetatm_bonds, pdb_hetatm_bonds, air=False, dir=mc_dir, blocks=config_data['atoms'])
    ...
    if config_data["by_chain"] and x:
        ...
        for chain, num in zip(enumerate(pdbm.enumerate_chains(space_filling_master_df)), cycle_sequence):
            ...
            mcf.create_nbt(bonds, pdb_backbone, air=False, dir=mc_dir, blocks=config_data['atoms'])
...
###
        ...
        mcf.create_nbt(master_bool_df, pdb_atoms, air=False, dir=mc_dir, blocks=config_data['atoms'])
