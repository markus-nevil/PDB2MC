...
def validate():
    ...
    if type(preset_file) == str:
        ...
        if not pdbm.check_model_size(preset_file, world_max=320):
            ...
            # Return the max size to the user
            return size_factor
...
if __name__ == '__main__':
    ...
    @app.route("/validate", methods=['POST'])(validate)
    @app.route("/custom")
    def custom():
        return "<p>Hello, custom </p>"
    @app.route("/skeleton")
    def skeleton():
        return "<p>Hello, skeleton </p>"
    @app.route("/space-filling")
    def spaceFilling():
        return "space filling"
    @app.route("/x-ray")
    def xRay():
        return "X-ray"
    @app.route("/amino-acids")
    def aminoAcids():
        return "amino acids"
    @app.route("/ribbon")
    def ribbon():
        ribbon.run_mode(pdb_name, pdb_file, rounded, mc_dir, config_data, hetatom_df, hetatm_bonds)
        return "<p>Hello, ribbon </p>"
